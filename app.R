#Edit line 174, Rdata, add instructions
...
server <- function(input, output,session) {
    observeEvent(input$instruct,{
        showModal(modalDialog(paste('Select the required classification method and the variable selection method along with the number of variables to be used from the dropdown boxes. You can edit the table displayed below or add new records and predict the species of the recordings'),footer = modalButton('Okay')))
    })
    ...
    output$intro<-renderUI({HTML(paste('Classifying the species of bats based on the information recorded on the sounds generated by bats.',
    ...
    output$tech<-renderUI({HTML('Classification Methods used:<br>LDA<br>QDA<br>KNN<br>Random Forest<br>SVM<br><br>Variable Selection Methods Used:<br>Boruta<br>Regsubsets')})
